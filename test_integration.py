...
class TestTopDownIntegration:
    ...
    def test_complete_transaction_workflow(self, clean_app, temp_data_file):
        ...
        for _, row in expense_stats.iterrows():
            print(f"  {row['分类']}: ¥{row['金额']} ({int(row['笔数'])}笔) - {row['占比']:.1f}%")
    ...
    def test_transaction_modification_workflow(self, clean_app):
        ...
        correct_transactions = [
            clean_app.add_transaction(100.0, TransactionType.INCOME, '工资💰', '兼职收入'),
            clean_app.add_transaction(50.0, TransactionType.EXPENSE, '交通🚗', '公交卡'),
        ]
        print(f"✅ 添加正确交易: 2笔")
        ...
        for idx, row in df.iterrows():
            print(f"  {row['ID'][:8]}... - {row['类型']} - {row['分类']} - ¥{row['金额']}")
...
class TestDataConsistencyIntegration:
    ...
    def test_large_volume_data_integration(self, clean_app):
        ...
        for day in range(30):
            ...
            if day % 7 == 0:  # 每周一次工资
                amount = 2000.0
                clean_app.add_transaction(amount, TransactionType.INCOME, '工资💰', f'第{day}天工资')
                total_income += amount
    ...
    def test_edge_cases_integration(self, clean_app):
        ...
        # 场景2: 极小金额的多次操作
        for i in range(100):
            clean_app.add_transaction(0.01, TransactionType.INCOME, '工资💰', f'小额收入{i}')
        ...
        # 场景3: 零余额状态的统计
        # 清空余额
        clean_app.add_transaction(balance_after_small, TransactionType.EXPENSE, '其他📦', '清零')
        ...
        print(f"  统计结果: 收入¥{summary['total_income']:.2f}, 支出¥{summary['total_expense']:.2f}")
...
def test_end_to_end_monthly_report(clean_app):
    ...
    for _, row in income_stats.iterrows():
        print(f"  {row['分类']}: ¥{row['金额']:,.2f} ({row['占比']:.1f}%) - {int(row['笔数'])}笔")
    ...
    for idx in range(min(5, len(df))):
        row = df.iloc[idx]
        print(f"    {row['日期'].strftime('%Y-%m-%d')} - {row['类型']} - {row['分类']} - ¥{row['金额']:,.2f}")
...
class TestComplexScenarios:
    ...
    def test_budget_tracking_scenario(self, clean_app):
        ...
        # 餐饮：每天30元，共900元
        for day in range(30):
            clean_app.add_transaction(30.0, TransactionType.EXPENSE, '餐饮🍜', f'第{day+1}天餐饮')
        
        # 交通：每周100元，共400元
        for week in range(4):
            clean_app.add_transaction(100.0, TransactionType.EXPENSE, '交通🚗', f'第{week+1}周交通')
        ...
        for _, row in expense_stats.iterrows():
            ...
            if budget > 0:
                ...
                diff = actual - budget
                print(f"{category:<10} ¥{budget:>8.2f} ¥{actual:>8.2f} {status:>6} ({diff:+.2f})")
        ...
        # 购物应该超预算（1800 > 1500）
        shopping_expense = expense_stats[expense_stats['分类'] == '购物🛒']['金额'].iloc[0]
