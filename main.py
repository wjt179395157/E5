...
def show_dashboard():
    ...
    with col1:
        st.metric("💰 当前余额", f"¥{balance:,.2f}", 
                 delta="账户总额", delta_color="off")
    
    # 30天统计
    summary_30 = app.get_summary(30)
    
    with col2:
        st.metric("📈 30天收入", f"¥{summary_30['total_income']:,.2f}",
                 delta=f"{summary_30['count']}笔交易")
    
    with col3:
        st.metric("📉 30天支出", f"¥{summary_30['total_expense']:,.2f}",
                 delta=f"-¥{summary_30['total_expense']:,.2f}", delta_color="inverse")
    
    with col4:
        net = summary_30['balance']
        st.metric("💵 30天净收入", f"¥{net:,.2f}",
                 delta="收入-支出", delta_color="normal" if net >= 0 else "inverse")
    ...
    with col1:
        ...
        if not expense_stats.empty:
            fig = px.pie(expense_stats, values='金额', names='分类',
                        title='近30天支出分类占比',
    ...
    with col2:
        ...
        if not income_stats.empty:
            fig = px.pie(income_stats, values='金额', names='分类',
                        title='近30天收入分类占比',
...
def show_add_transaction():
    ...
    with col1:
        ...
        if st.button("✅ 提交记录", type="primary", use_container_width=True):
            try:
                trans_type_enum = TransactionType.EXPENSE if trans_type == "支出" else TransactionType.INCOME
                app.add_transaction(st.session_state["amount"], trans_type_enum, category, note)
                st.success(f"✅ 记账成功！{trans_type} ¥{st.session_state['amount']:.2f}")
                st.balloons()
...
def show_transactions():
    ...
    # 应用筛选
    df_filtered = df[
        (df['类型'].isin(filter_type)) & 
        (df['分类'].isin(filter_category))
    ].head(limit)
