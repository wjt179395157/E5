...
class TestAddTransaction:
    ...
    def test_add_transaction_without_note(self, app):
        """测试用例3: 添加交易时不提供备注"""
        result = app.add_transaction(
            amount=50.0, trans_type=TransactionType.EXPENSE, category="交通🚗"
        )
    ...
    def test_add_transaction_zero_amount(self, app):
        ...
        with pytest.raises(ValueError, match="金额必须大于0"):
            app.add_transaction(
                amount=0.0, trans_type=TransactionType.EXPENSE, category="餐饮🍜"
            )
    ...
    def test_add_transaction_negative_amount(self, app):
        ...
        with pytest.raises(ValueError, match="金额必须大于0"):
            app.add_transaction(
                amount=-100.0, trans_type=TransactionType.INCOME, category="工资💰"
            )
